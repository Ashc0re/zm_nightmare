...
function __init__()
{
	zm_powerups::register_powerup( "strange_idol", &grab_strange_idol );
	if( ToLower( GetDvarString( "g_gametype" ) ) != "zcleansed" )
	{
		zm_powerups::add_zombie_powerup( "strange_idol",	"p7_zm_power_up_widows_wine", &"ZOMBIE_POWERUP_WW_GRENADE",	&zm_powerups::func_should_always_drop, POWERUP_ONLY_AFFECTS_GRABBER, !POWERUP_ANY_TEAM, !POWERUP_ZOMBIE_GRABBABLE );
		zm_powerups::powerup_set_player_specific( "strange_idol", POWERUP_FOR_SPECIFIC_PLAYER ); 
	}
}

function grab_strange_idol( player )
{	
	//level thread ww_grenade_powerup( self, player );
	player.hasIdol = true;
	IPrintLn( "Picked up Strange Idol" );
